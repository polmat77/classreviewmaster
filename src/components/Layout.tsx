...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const steps = [
    ...
    {
      id: 2,
      name: "Appréciation générale de classe",
      path: "/appreciation-generale",
      icon: MessageCircle,
    },
    {
      id: 3,
      name: "Appréciations individuelles",
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <footer className="border-t border-border/40 py-6 bg-secondary/50">
        <div className="app-container text-center text-sm text-muted-foreground">
          <p>
            © {new Date().getFullYear()} ClassReviewMaster — Simplifiez
            l'analyse de vos bulletins scolaires
          </p>
        </div>
      </footer>
    </div>
