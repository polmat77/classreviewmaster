...
const analyzeGradesFormat = (rawData: any[]): { eleves: Student[], classeInfo: any, subjects: string[] } => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^[A-Z]+\s+[A-Za-zÀ-ÿ]+\s+\d/) || 
        line.match(/^\d+\s+[A-Z]+\s+[A-Za-zÀ-ÿ]+/)) {
      dataStartIndex = i;
      break;
    }
  }
  ...
  // Identifier les matières basées sur l'en-tête du tableau
  // Chercher la ligne d'en-tête juste avant les données
  let headerLine = null;
  for (let i = dataStartIndex - 1; i >= 0; i--) {
    const line = Array.isArray(lines[i]) ? lines[i] : [String(lines[i])];
    if (line.length > 5) { // L'en-tête doit avoir plusieurs colonnes
      headerLine = line;
      break;
    }
  }

  // Matières par défaut si l'en-tête n'est pas trouvé
  let subjectsMap = [
    "FRANÇAIS",
    "MATHÉMATIQUES", 
    "HISTOIRE-GÉOGRAPHIE",
  ...
  // Si on a trouvé un en-tête, l'utiliser pour identifier les matières
  if (headerLine && headerLine.length > 3) {
    subjectsMap = headerLine.slice(3).map(subject => 
      typeof subject === 'string' ? subject.toUpperCase() : String(subject).toUpperCase()
    );
  }
  ...
  for (const line of elevesLines) {
    ...
    // Extraire le nom et prénom (gérer le cas où il y a un numéro en première position)
    let nom, prenom, classeEleve;
    let notesStartIndex;

    if (lineArray[0].match(/^\d+$/)) {
      // Format: numéro nom prénom classe
      nom = lineArray[1];
      prenom = lineArray[2]; 
      classeEleve = lineArray[3];
      notesStartIndex = 4;
    } else {
      // Format: nom prénom classe
      nom = lineArray[0];
      prenom = lineArray[1];
      classeEleve = lineArray[2];
      notesStartIndex = 3;
    }

    // Extraire les notes
    const notes = lineArray.slice(notesStartIndex).map(note => {
      if (note === 'Abs' || note === '' || note === null) return null;
      const cleanedNote = String(note).replace(',', '.').trim();
      const parsedValue = parseFloat(cleanedNote);
